<header class={`text-fg p-3 bg-ghostbg pl-5 pr-5 flex flex-row items-center justify-between`}>
  <a href="/" class="bg-ghostbg p-2 pl-5 pr-5 font-black text-xl rounded-md hover:text-primary transition-all">
      <img src="../fuujin.svg" alt="" class="w-12 h-auto inline align-middle"/>
    Fuujin</a>

  <nav class="flex flex-row items-center justify-center gap-x-5 text-xl p-2  pr-4 pl-4 bg-ghostbg rounded-md">
    <a class="hover:font-bold hover:text-secondary transition-all" href="/playground">Playground</a>
    <a class="hover:font-bold hover:text-secondary transition-all" href="/learn">Learn</a>
  </nav>
</header>

<style>
</style>

